function [A_cell,B_cell,varargout] = parallel_bilinear_dynamic_inference(X, D, F, infer_hand, inf_opts, varargin)
...
%for kk = 1:numel(X)
    ...
    % if nargout > 2
    %     [A_cell{kk},B_cell{kk},costForBIC] = feval(infer_hand, X{kk}, D, F, inf_opts);    % Perform inference over each sequence independently
    % else
    %     [A_cell{kk},B_cell{kk}] = feval(infer_hand, X{kk}, D, F, inf_opts);    % Perform inference over each sequence independently
    % end
end
...
for kk = 1:numel(X)
    ...
    % if nargout > 2
    %     varargout{1}{kk} = reshape(costForBIC{kk},size(costForBIC{kk},1),size(costForBIC{kk},3));% Reshape dynamics coefficients
    % end
end
