function [A,B] = bpdndf_bilinear_handle_behavior_x(X,D,F,opts, varargin)
...
if opts.behaviordLDS == 1
    if strcmp(opts.special,'noobs')
        % [A, ~] = BPDN_DF_bilinearNoObs_behavior_x(X, MEAS_FUN, F, DWTfunc, param_vals,Psi,behavior);
        [A, ~] = BPDN_DF_bilinearNoObs(X, MEAS_FUN, F, DWTfunc, param_vals);
        B = A{2};
        A = A{1};
    else
        [A, ~] = BPDN_DF_bilinear_behavior_x(X, MEAS_FUN, F, DWTfunc, param_vals,Psi,behavior);
        B = A{2};
        A = A{1};
    end
elseif strcmp(opts.special,'sysop')
