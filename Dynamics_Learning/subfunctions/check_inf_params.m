...
if (~isfield(inf_opts,'verysparsebhv'))||isempty(inf_opts.verysparsebhv)
    inf_opts.verysparsebhv = false;
end
if (~isfield(inf_opts,'psinorm'))||isempty(inf_opts.psinorm)
    inf_opts.psinorm = 'norm'; %alternative: 'frob'
end
if (~isfield(inf_opts,'lambda2'))||isempty(inf_opts.lambda2)
    inf_opts.lambda2 = 1e-5;
end
if (~isfield(inf_opts,'fewerneurpercol'))||isempty(inf_opts.fewerneurpercol)
    inf_opts.fewerneurpercol = false;
end
if (~isfield(inf_opts,'lambda_D'))||isempty(inf_opts.lambda_D)
    inf_opts.lambda_D = 1e-5;
end
if (~isfield(inf_opts,'Dnorm'))||isempty(inf_opts.Dnorm)
    inf_opts.Dnorm = 'norm'; %alternative: 'frob'
end
