...
inf_opts = checkSizes(inf_opts, D_init, F_init, data_obj, data_type, sig_opts);      % Check the sizes of all the initializations % EY added data_obj, data_type, sig_opts 1/16/2026 in response to error
...
for n_iters = 1:inf_opts.max_iters
    ...
    if inf_opts.AcrossIndividuals % EY added 08/27/2023
        ...
        [A_cell,B_cell] = parallel_bilinear_dynamic_inference(X_ex, D, F, ...
                                       @bpdndf_bilinear_handle, inf_opts); % Infer sparse coefficients
        % [A_cell,B_cell,costForBIC] = parallel_bilinear_dynamic_inference(X_ex, D, F, ...
        %                                @bpdndf_bilinear_handle, inf_opts); % Infer sparse coefficients
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% Update step
    ...
    elseif strcmp(data_type, 'datacell')             
        ...
        fprintf('Iteration %d complete. Mean dD: %f, Mean dF: %f, Max b: %f, Median b use: %f\n', n_iters, ...
            mean(sum((D - D_old).^2)./sum(D_old.^2)), f_err, max(max(abs(cell2mat(B_cell)))), ...
            median(sum(abs(cell2mat(B_cell))>1e-3)) )
        % BIC = median(sum(abs(cell2mat(B_cell))>1e-3))*log(size(inf_opts.N_ex,1))+2*log(costForBIC);
        % fprintf('BIC: %f',BIC)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function inf_opts = checkSizes(inf_opts, D_init, F_init, varargin) % EY added varargin 1/16/26
