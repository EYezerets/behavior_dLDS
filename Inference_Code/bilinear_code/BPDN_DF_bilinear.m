...
if isfield(param_vals, 'solver_type')
    if strcmp(param_vals.solver_type, 'cvx')
        ...
        doTFOCS = false;
        doFISTA = false;
    elseif strcmp(param_vals.solver_type, 'fista')
        doCVX = false;
        doTFOCS = false;
        doFISTA = true;
    else
...
resFISTA = zeros(N2+N_b,num_frames); % n+nF
resTFOCS = zeros(N2+N_b,num_frames);
...
for kk = 2:num_frames
    ...
    if doTFOCS
        ...
        Af = @(x) [sqrt(lambda_historyb)*x(N2+1:N2+N_b); ...
               ...
               Phi(DWT_invert(x(1:N2)))]; % based on indices in X, the thing you're solving for (i.e., zeros(N2+N_b, 1)) (Y = AX)
        Ab = @(x) [DWT_apply(Phit(x((N_b+N2+1):end))) + sqrt(lambda_history)*x((N_b+1):(N_b+N2)); ...
                   sqrt(lambda_historyb)*x(1:N_b) - sqrt(lambda_history)*(f_dyn')*x((N_b+1):(N_b+N2))]; % based on the indices in Y (i.e., [sqrt(lambda_historyb)*bcoef_dcs(:,:,kk-1); zeros(N2,1); MEAS_SIG(:, :, kk)])
    ...
    %     fprintf('condition of F is %f\n', cond(f_dyn))
        ...
        % writematrix(res,'res_TFOCS.txt')
        % filename=('resTFOCS.mat'); 
        % m = matfile(filename);
        % varToSave = res.';
        % s = size(m, 'varToSave');
        % m.my_variables(s+1, :) = varToSave;
        % resTFOCS(:,kk) = res;
    end
    if doCVX
        n_x = N2;
    ...
    if doFISTA 
        ...
        % opts.tol = 1e-6; % hardcoded

        if deltaOpt;  yNow = MEAS_SIG(:,:,kk) - MEAS_SIG(:,:,kk-1);
        else;         yNow = MEAS_SIG(:,:,kk);                        end
        
        if lambda_historyb == 0
            A = [sqrt(lambda_history)*eye(N2,N2) sqrt(lambda_history)*-1*f_dyn;whatIsD zeros(size(whatIsD,1),N_b)];
            Y = [zeros(N2,1); yNow];
            lamVec = [lambda_val*ones(N2,1);lambda_b*ones(N_b,1)];
            ...
            opts.lambda     = lamVec(:);
            opts.check_grad = 0;    
            % [res,costForBIC] = fista_lasso(Y, A, [], opts); %FIXME: augmented Y needed, not just measurement
        ...
        % writematrix(res,'res_FISTA.txt')
        % filename=('res_FISTA.mat'); 
        % m = matfile(filename);
        % varToSave = res.';
        % s = size(m, 'varToSave');
        % m.my_variables(s+1, :) = varToSave;
...
% tiledlayout('flow');nexttile();hold on;title('FISTA coeffs');imagesc(resFISTA);colorbar;hold off;nexttile();hold on;title('TFOCS coeffs');imagesc(resTFOCS);colorbar;hold off;nexttile();hold on;title('FISTA-TFOCS');imagesc(resFISTA-resTFOCS);colorbar;hold off;
% pause();
% disp(sum(abs(resTFOCS-resFISTA)))
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Set ouptputs
