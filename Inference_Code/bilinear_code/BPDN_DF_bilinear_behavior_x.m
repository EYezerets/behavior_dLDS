function [varargout] = BPDN_DF_bilinear_behavior_x(varargin)
    ...
    if ifBehavior
        ...
        if doTFOCS
            ...
            Af = @(x) [sqrt(lambda_historyb)*x(N2+1:N2+N_b); ...
                   ...
                   sqrt(lambda_behavior)*Psi*x(1:N2)];
            Ab = @(x) [DWT_apply(Phit(x((N_b+N2+1):(N_b+N2+size(yNow,1))))) + sqrt(lambda_history)*x((N_b+1):(N_b+N2)) + sqrt(lambda_behavior)*DWT_apply(PsitFunc(x((N_b+N2+size(yNow,1)+1):end))); ...
                       sqrt(lambda_historyb)*x(1:N_b) - sqrt(lambda_history)*(f_dyn')*x((N_b+1):(N_b+N2))];
            A = linop_handles([M+N2+N_b+nBhv, N2+N_b], Af, Ab, opt_set); % A transpose
        ...
        %     fprintf('condition of F is %f\n', cond(f_dyn))
            ...
            res    = solver_L1RLS(A, [sqrt(lambda_historyb)*bcoef_dcs(:,:,kk-1); zeros(N2,1); yNow; sqrt(lambda_behavior)*thisBehaviorSample(:,kk)], lamVec, zeros(N2+N_b, 1), opts );
            % resTFOCS(:,kk) = res;
        end
        ...
        if doFISTA
            ...
            if lambda_historyb == 0
                ...
                A = [sqrt(lambda_history)*eye(N2,N2) sqrt(lambda_history)*-1*f_dyn;whatIsD zeros(size(whatIsD,1),N_b); zeros(size(whatIsPsi,1),N_b) sqrt(lambda_behavior)*whatIsPsi];
                
                Y = [zeros(N2,1); yNow; sqrt(lambda_behavior)*thisBehaviorSample(:,kk)]; % add on nBhv by 1
                ...
                A = [zeros(N_b,N2) sqrt(lambda_historyb)*eye(N_b,N_b); sqrt(lambda_history)*eye(N2,N2) sqrt(lambda_history)*-1*f_dyn;whatIsD zeros(size(whatIsD,1),N_b);zeros(size(whatIsPsi,1),N_b) sqrt(lambda_behavior)*whatIsPsi];
                Y = [sqrt(lambda_historyb)*bcoef_dcs(:,:,kk-1); zeros(N2,1); yNow; sqrt(lambda_behavior)*thisBehaviorSample(:,kk)];
                lamVec = [lambda_val*ones(N2,1);lambda_b*ones(N_b,1)];
        ...
        error('use a different function if not using behavior-dLDS')
