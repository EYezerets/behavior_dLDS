    ...
    if ifBehavior
        ...
        if doTFOCS
            ...
            Af = @(x) [sqrt(lambda_historyb)*x(N2+1:N2+N_b); ...
                   ...
                   sqrt(lambda_behavior)*Psi*x(N2+1:N2+N_b)];
            Ab = @(x) [DWT_apply(Phit(x((N_b+N2+1):(N_b+N2+size(yNow,1))))) + sqrt(lambda_history)*x((N_b+1):(N_b+N2)); ...
                       sqrt(lambda_behavior)*DWT_apply(PsitFunc(x((N_b+N2+size(yNow,1)+1):end))) + sqrt(lambda_historyb)*x(1:N_b) - sqrt(lambda_history)*(f_dyn')*x((N_b+1):(N_b+N2))];
        ...
        if doFISTA
            ...
            if lambda_historyb == 0
                ...
                A = [sqrt(lambda_history)*eye(N2,N2) sqrt(lambda_history)*-1*f_dyn;whatIsD zeros(size(whatIsD,1),N_b); zeros(size(whatIsPsi,1),N2) sqrt(lambda_behavior)*whatIsPsi];
